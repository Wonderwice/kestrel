...
struct HitRecord {
    ...
    Vec3 normal;       ///< Surface normal at intersection (always points outward from surface)
 ...
 * The sphere is defined implicitly as all points p where |p - center|² = radius².
 */
class Sphere {
public:
     ...
     * @return True if intersection exists in range [t_min, t_max], false otherwise
     * 
     ...
     */
    HOST_DEVICE bool hit(const Ray& ray, float t_min, float t_max, HitRecord& rec) const {
