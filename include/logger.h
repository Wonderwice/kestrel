...
/**
 * @brief Exception thrown for fatal Kestrel errors (scene/IO failures).
 *        Caught at the top level so destructors run during unwinding.
 */
class KestrelError : public std::runtime_error {
public:
  explicit KestrelError(const std::string &what) : std::runtime_error(what) {}
};
...
public:
  ...
  static Logger *get_instance() {
    static Logger instance; // thread-safe initialisation since C++11
    return &instance;
  }
...
// Convenience macros for logging with file and line information. Fully qualified
// so they work from both global scope (main) and inside namespace kestrel.
#define LOG_DEBUG(msg) ::kestrel::Logger::get_instance()->log(::kestrel::LogLevel::DEBUG, msg, __FILE__, __LINE__)
#define LOG_INFO(msg) ::kestrel::Logger::get_instance()->log(::kestrel::LogLevel::INFO, msg, __FILE__, __LINE__)
#define LOG_WARNING(msg) ::kestrel::Logger::get_instance()->log(::kestrel::LogLevel::WARNING, msg, __FILE__, __LINE__)
// Fatal: log then throw so stack unwinding runs destructors. Caught in main().
#define LOG_ERROR(msg) do { ::kestrel::Logger::get_instance()->log(::kestrel::LogLevel::ERROR, msg, __FILE__, __LINE__); throw ::kestrel::KestrelError(msg); } while(0)
